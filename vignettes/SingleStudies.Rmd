...
- **Case**: a period of continuous observation of a person containing one or more outcomes. One person can contribute to multiple cases, e.g. when the person has multiple observation periods.
...
- **Study period**: a period of calendar time when persons are eligible to be included as cases. For example, a study period could be January 1, 2020, onward. Unlink nesting cohorts, which could specify different time periods per person, a study period applies to all persons. 
...
We need to define the exposure and outcomes for our study. For the exposure, we will directly use the `drug_era` table in the CDM. For the outcome we can use the OHDSI `PhenotypeLibrary` to retrieve a community-approved definition of epistaxis:
...
We see some effect for season: epistaxis tends to be more prevalent during winter. We should verify if our model accounts for time trends, by plotting the rate of the outcome across time, before and after adjustment:
...
```{r echo=FALSE,message=FALSE}
if (folderExists) {
  plot <- readRDS(file.path(folder, "stabilityPlot.rds"))
  print(plot)
}
```

Here we see that after adjustment, in general, the rate of outcome appears fairly stable across time, with the exception of some months in 2020. This is what we refer to as the 'COVID-blip', because during the time of the COVID-19 pandemic regular healthcare was put on hold. Our time trend diagnostic confirms the rate of the outcome was significantly lower during these months:
...
```{r echo=FALSE,message=FALSE}
if (folderExists) {
  stability <- readRDS(file.path(folder, "stability.rds"))
  stability %>%
    filter(!stable)
}
```
...
```{r echo=FALSE,message=FALSE}
if (folderExists) {
  plot <- readRDS(file.path(folder, "stabilityPlot2.rds"))
  print(plot)
}
```

We can verify if this improved temporal stability:
...
```{r echo=FALSE,message=FALSE}
if (folderExists) {
  stability <- readRDS(file.path(folder, "stability2.rds"))
  stability %>%
    filter(!stable)
}
```
