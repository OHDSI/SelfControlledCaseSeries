...
x$summary$coefficients <- c(0.8438903)
x$modelfit <- matrix(c(-824.1930,-824.1930,-824.1930,-824.1930), nrow = 2)
...
#   clogit(y ~ covariateValue + strata(stratumId) + offset(log(time)), data = myData)
#
#   sccsData = data.frame(Indiv = as.factor(data$personId),
#                     aevent = data$ageInDays + data$eventDate - data$observationStartDate,
#                     astart = data$ageInDays,
#                     aend = data$ageInDays + data$censorDate - data$observationStartDate + 1,
#                     aestudy = data$ageInDays + observationDays)
