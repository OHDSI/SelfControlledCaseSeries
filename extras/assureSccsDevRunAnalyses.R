...
createReferenceTable <- function(sccsAnalysisList,
  ...
  for (sccsAnalysis in sccsAnalysisList) {
    for (exposureOutcome in exposureOutcomeList) {
      ...
      instantiatedExposureOutcome$trueEffectSize <- .selectByType(sccsAnalysis$trueEffectSizeType, exposureOutcome$trueEffectSize, "trueEffectSize")
      ...
      row <- tibble(
        ...
        trueEffectSize = instantiatedExposureOutcome$trueEffectSize,
...
summarizeSccsAnalyses <- function(referenceTable, outputFolder) {
  columns <- c("analysisId", "exposureId", "outcomeId", "trueEffectSize")
