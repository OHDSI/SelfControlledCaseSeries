...
data <- data.frame(age = rep(age, 2), rr = c(rr, targetRr), group = rep(c("Estimated", "Simulated"),
                                                                        each = length(rr)))
...
ggplot2::ggplot(data, ggplot2::aes(x = age,
                                   y = rr,
                                   group = group,
                                   color = group)) + ggplot2::geom_hline(yintercept = breaks,
                                                                                                         colour = "#AAAAAA",
                                                                                                         lty = 1,
                                                                                                         lw = 0.2) + ggplot2::geom_line() + ggplot2::scale_x_continuous("Age",
                                                                                                                                                                        breaks = ageBreaks,
                                                                                                                                                                        labels = ageLabels) + ggplot2::scale_y_continuous("Relative risk", lim = c(0.1, 10), trans = "log10", breaks = breaks, labels = breaks) + ggplot2::theme(panel.grid.minor = ggplot2::element_blank(), panel.background = ggplot2::element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = ggplot2::element_blank(), axis.ticks = ggplot2::element_blank(), axis.text.y = themeRA, axis.text.x = theme, strip.text.x = theme, strip.background = ggplot2::element_blank(), legend.title = ggplot2::element_blank(), legend.position = "top")
allCoefs <- coef(model)
coefId <- as.numeric(names(allCoefs))
...
theme <- ggplot2::element_text(colour = "#000000", size = 12)
themeRA <- ggplot2::element_text(colour = "#000000", size = 12, hjust = 1)
ggplot2::ggplot(data, ggplot2::aes(x = season,
                                   y = rr,
                                   group = group,
                                   color = group)) + ggplot2::geom_hline(yintercept = breaks,
                                                                                                            colour = "#AAAAAA",
                                                                                                            lty = 1,
                                                                                                            lw = 0.2) + ggplot2::geom_line() + ggplot2::scale_x_continuous("Month",
                                                                                                                                                                           breaks = seasonBreaks,
                                                                                                                                                                           labels = seasonBreaks) + ggplot2::scale_y_continuous("Relative risk", lim = c(0.1, 10), trans = "log10", breaks = breaks, labels = breaks) + ggplot2::theme(panel.grid.minor = ggplot2::element_blank(), panel.background = ggplot2::element_rect(fill = "#FAFAFA", colour = NA), panel.grid.major = ggplot2::element_blank(), axis.ticks = ggplot2::element_blank(), axis.text.y = themeRA, axis.text.x = theme, strip.text.x = theme, strip.background = ggplot2::element_blank(), legend.title = ggplot2::element_blank(), legend.position = "top")
