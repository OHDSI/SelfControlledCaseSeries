...
createEraCovariateSettings <- function(includeEraIds = NULL,
  ...
  if (is.character(trueEffectSize)) {
    checkmate::assertCharacter(trueEffectSize, len = 1, add = errorMessages)
  } else {
    checkmate::assertIntegerish(trueEffectSize, null.ok = TRUE, add = errorMessages)
  }
