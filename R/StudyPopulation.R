...
#'
#' @param sccsData              An object of type \link{SccsData} as created using the
...
createStudyPopulation <- function(sccsData,
  ...
  if (naivePeriod != 0) {
    ...
    outcomes <- outcomes %>%
      inner_join(select(cases, .data$observationPeriodId, .data$startAgeInDays, .data$ageInDays), by = "observationPeriodId") %>%
  ...
  if (!is.null(minAge) || !is.null(maxAge)) {
    ...
    if (!is.null(maxAge)) {
      maxAgeInDays <- (maxAge + 1) * 365.25
      cases <- cases %>%
        mutate(endAgeInDays = case_when(.data$endAgeInDays > maxAgeInDays ~ maxAgeInDays,
    ...
    outcomes <- outcomes %>%
      inner_join(select(cases, .data$observationPeriodId, .data$startAgeInDays, .data$endAgeInDays, .data$ageInDays), by = "observationPeriodId") %>%
      filter(.data$startDay >= .data$startAgeInDays - .data$ageInDays &
  ...
  cases <- cases %>%
    ...
    select(.data$observationPeriodId, .data$personId, .data$startDate, .data$endDay, .data$ageInDays, .data$offset)

  outcomes <- outcomes %>%
    inner_join(select(cases, .data$observationPeriodId, .data$offset), by = "observationPeriodId") %>%
  ...
  return(list(outcomes = outcomes, cases = cases, metaData = metaData))
}

countOutcomes <- function(outcomes, cases, description) {
  counts <- outcomes %>%
    inner_join(cases, by = "observationPeriodId") %>%
    group_by(.data$eraId) %>%
